...
스키와 스노보드 동작을 영상 기반으로 분석하기 위한 프로그램입니다. opencv로 객체를 추출한 뒤 MediaPipe Pose로 관절 좌표를 추출하고, 정규화와 각도 계산을 통해 동작 차이를 비교해서 데모와 사용자의 영상간의 차이를 요약하여 LLM에게 전송 후 피드백을 받는 프로그램입니다.
...
LLM 피드백을 사용하려면 다음 환경변수를 env에 설정합니다.
...
- `feedback_engine.py`는 스키/스노보드의 레벨 1~3 기술 목록을 포함합니다.
